...
$.event.special.swipe={setup:setup};$.each(['left','up','down','right'],function(){$.event.special['swipe'+this]={setup:function(){$(this).on('swipe',$.noop);}};});}));const NO_MESSAGES_HINT=`¡Nada por acá!
     <br>
     <br>
     Pasále el link a tus amigos y empezá a recibir mensajes.`;const NO_INTERNET_HINT='No tenés conexión a internet.';const BACK_ONLINE_HINT='Ya tenés internet.';let isPrivate=!1;let loader=()=>{console.info('loader: no loader was specified.')};let deferredPreloader=null;let viewportThreshold=(VIEWPORT_VISIBLE_THRESHOLD*$(window).height())/100;let isOnline=navigator.onLine;let heartbeatLooper=null;let canPullChunks=!0;let deferredFetcher=null;let reportAdImpressions=!0;let markdownConverter=null;let loginTarget=null;let gotHistoryPushState=!1;let userNavBackground=null;const FONTS=[{family:'Lato',weight:300,src:'url(https://fonts.gstatic.com/s/lato/v17/S6u9w4BMUTPHh7USSwaPGR_p.woff2) format(\'woff2\')'},{family:'Lato',weight:300,src:'url(https://fonts.gstatic.com/s/lato/v17/S6u9w4BMUTPHh7USSwiPGQ.woff2) format(\'woff2\')'},{family:'Lato',weight:400,src:'url(https://fonts.gstatic.com/s/lato/v17/S6uyw4BMUTPHjxAwXjeu.woff2) format(\'woff2\')'},{family:'Lato',weight:400,src:'url(https://fonts.gstatic.com/s/lato/v17/S6uyw4BMUTPHjx4wXg.woff2) format(\'woff2\')'},{family:'Lato',weight:700,src:'url(https://fonts.gstatic.com/s/lato/v17/S6u9w4BMUTPHh6UVSwaPGR_p.woff2) format(\'woff2\')'},{family:'Lato',weight:700,src:'url(https://fonts.gstatic.com/s/lato/v17/S6u9w4BMUTPHh6UVSwiPGQ.woff2) format(\'woff2\')'},{family:'Roboto',weight:300,src:'url(https://fonts.gstatic.com/s/roboto/v20/KFOlCnqEu92Fr1MmSU5fCRc4EsA.woff2) format(\'woff2\')'},{family:'Roboto',weight:300,src:'url(https://fonts.gstatic.com/s/roboto/v20/KFOlCnqEu92Fr1MmSU5fABc4EsA.woff2) format(\'woff2\')'},{family:'Roboto',weight:300,src:'url(https://fonts.gstatic.com/s/roboto/v20/KFOlCnqEu92Fr1MmSU5fCBc4EsA.woff2) format(\'woff2\')'},{family:'Roboto',weight:300,src:'url(https://fonts.gstatic.com/s/roboto/v20/KFOlCnqEu92Fr1MmSU5fBxc4EsA.woff2) format(\'woff2\')'},{family:'Roboto',weight:300,src:'url(https://fonts.gstatic.com/s/roboto/v20/KFOlCnqEu92Fr1MmSU5fCxc4EsA.woff2) format(\'woff2\')'},{family:'Roboto',weight:300,src:'url(https://fonts.gstatic.com/s/roboto/v20/KFOlCnqEu92Fr1MmSU5fChc4EsA.woff2) format(\'woff2\')'},{family:'Roboto',weight:300,src:'url(https://fonts.gstatic.com/s/roboto/v20/KFOlCnqEu92Fr1MmSU5fBBc4.woff2) format(\'woff2\')'},{family:'Roboto',weight:400,src:'url(https://fonts.gstatic.com/s/roboto/v20/KFOmCnqEu92Fr1Mu72xKOzY.woff2) format(\'woff2\')'},{family:'Robot',weight:400,src:'url(https://fonts.gstatic.com/s/roboto/v20/KFOmCnqEu92Fr1Mu5mxKOzY.woff2) format(\'woff2\')'},{family:'Roboto',weight:700,src:'url(https://fonts.gstatic.com/s/roboto/v20/KFOlCnqEu92Fr1MmWUlfCRc4EsA.woff2) format(\'woff2\')'},{family:'Roboto',weight:700,src:'url(https://fonts.gstatic.com/s/roboto/v20/KFOlCnqEu92Fr1MmWUlfABc4EsA.woff2) format(\'woff2\')'},{family:'Material Icons',weight:400,src:'url(https://fonts.gstatic.com/s/materialicons/v55/flUhRq6tzZclQEJ-Vdg-IuiaDsNc.woff2) format(\'woff2\')'}];let loadedFonts=[];FONTS.forEach((font)=>{let fontFace=new FontFace(font.family,font.src,{style:font.style,weight:font.weight});fontFace.load().then(()=>{loadedFonts.push(fontFace);if(loadedFonts.length==FONTS.length){console.info('FontFace: loaded',loadedFonts.length,'fonts (out of',FONTS.length,'required fonts).');loadedFonts.forEach((loadedFont)=>{document.fonts.add(loadedFont)})}})});jQuery.fn.fadeIn=function(callback=()=>{},opacity=1,duration=MATERIALIZE_TRANSITION_TIME){$(this).css({display:'',opacity:opacity});setTimeout(callback,duration)}
jQuery.fn.fadeOut=function(callback=()=>{},opacity=0){element=$(this);element.css({transition:'opacity '+(MATERIALIZE_TRANSITION_TIME/1000)+'s linear',opacity:opacity});setTimeout(()=>{if(element.css('display')!='none'){element.data('data-display',element.css('display'))}
element.css({display:'none'});callback()},MATERIALIZE_TRANSITION_TIME)}
function pushFakeHistory(){if(!gotHistoryPushState){history.pushState(null,document.title,location.href);gotHistoryPushState=!0}}
function displayNoInternetFAB(){$('#noInternetBtn').css({oapcity:0,display:'block'}).animate({opacity:0.5})}
function run(url,action,values,before=()=>{},overridesFailure=!1,xhr=()=>{return $.ajaxSettings.xhr()}){console.info('run \n\nurl:',url,'action:',action,'values:',values,'before:',before);return $.ajax({url:'services/'+url+'.php',type:'POST',processData:!1,contentType:'application/json',beforeSend:before,data:JSON.stringify({'action':action,'values':values}),xhr:xhr}).done((response,status,xhr)=>{if(!overridesFailure&&!xhr.getResponseHeader('content-type').includes('json')){console.error(response);if($('.toast').length<1){if(isOnline){toast('Algo salió mal, por favor probá otra vez.')}else{toast(NO_INTERNET_HINT)}}}}).fail((error)=>{if(!overridesFailure){console.error(error);if($('.toast').length<1){if(isOnline&&error.status!=0){toast('Algo salió mal, por favor probá otra vez.')}else{isOnline=!1;displayNoInternetFAB();toast(NO_INTERNET_HINT)}}}})}
...
function animateRedirect(url,fullBody=!1,timeout=null){setTimeout(()=>{redirCall=()=>{$(fullBody?'body':'main').fadeOut(()=>{redirect(url,0)})};sidenav=$('.sidenav');if(sidenav.length>0){sidenav=M.Sidenav.getInstance(sidenav);sidenav.close();setTimeout(redirCall,sidenav.options.outDuration)}else{redirCall()}},timeout=null?MATERIALIZE_TRANSITION_TIME:timeout)}
...
function setupGoogleAnalytics(){window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments)}
gtag('js',new Date());gtag('config',GOOGLE_ANALYTICS_KEY);console.info('setupGoogleAnalytics: success setting up Google Analytics.')}
...
function isElementInViewport(element){element=$(element);elementTop=element.offset().top;elementBottom=elementTop+element.outerHeight();viewportTop=$(window).scrollTop();viewportBottom=viewportTop+$(window).height();return elementBottom>(viewportTop-viewportThreshold)&&(elementTop-viewportThreshold)<viewportBottom}
function getRenderedMessage(id,content,declaredName,created=null,display=!1,reported,image=null,verified=!0,comments=0,deferImage=!1,likes=0,position=-1){content=content.toString();return `<div
...
function openCommentsModal(message,private){loadRecaptcha();$('#commentsModal').modal('open');let card=$('.message[data-message="'+message+'"]');let image=card.find('img');$('#commentsMessageWrapper').html(`<ul class="collection border-dark-8">
         ...
         <ul id="commentsCollapsible" class="collapsible border-dark-8" style="display: none;"> </ul>`);$('#commentInput').on('change keyup keydown',function(){if($(this).val().length>0){$(this).removeClass('invalid').addClass('valid')}else{$(this).addClass('invalid').removeClass('valid')}});$('#commentInput, #commentDeclaredName').on('change keyup',(event)=>{if(event.ctrlKey&&event.key=='Enter'){$('#sendCommentBtn').click()}});$('#sendCommentBtn').on('click',()=>{content=$('#commentInput').val();declaredName=$('#commentDeclaredName').val();declaredName=declaredName.length>0?declaredName:null;if(content.length>0){if(isOnline){run('commentsManager','postComment',{content:content,declaredName:declaredName,message:message,private:private||(typeof(RECIPIENT)!='undefined'&&RECIPIENT!=null)},()=>{disable($('#commentInput, #commentDeclaredName, #sendCommentBtn'))}).done((response)=>{console.info(response);if(response.status==OK){console.info('postComment: alright, we got a response!');$('#commentInput, #commentDeclaredName').val('').change();console.info('postComment: now, we\'ll clean up those inputs.');commentsCollapsible=$('#commentsCollapsible');instance=M.Collapsible.getInstance($('#commentsCollapsible'));for(let index=0;index<commentsCollapsible.find('li').length;index++){instance.close(index)}
console.info('postComment: ok, we have re-initialized the collapsible and closed its items.');commentsCollapsible.find('.active').removeClass('active');commentsCollapsible.prepend(getRenderedComment(response.result.id,declaredName,content));console.info('postComment: cool, we got the DOM changed.');commentsCollapsible.collapsible({accordion:!1});commentsCollapsible=$('#commentsCollapsible');instance=M.Collapsible.getInstance($('#commentsCollapsible'));instance.open(0);console.info('postComment: so we\'ve just re-initialized the instance back again, this is getting frustrating.');firstComment=commentsCollapsible.find('li').first();$('#commentsModal').find('.modal-content').animate({scrollTop:firstComment.offset()['top']-firstComment.find('.collapsible-header').height()});commentCount=$('.message[data-message='+message+']').find('.commentCount');commentCount.html(parseInt(commentCount.html())+1);console.info('postComment: finally, I did it, we did it, what we all waited for so long is finally here, we\'re done!');$('#commentInput').removeClass('invalid valid');toast('¡Listo! Publicaste tu comentario.')}else{toast('Algo anda mal, probá de nuevo.')}}).always(()=>{enable($('#commentInput, #commentDeclaredName, #sendCommentBtn'))})}else{if($('.toast').length<1){toast(NO_INTERNET_HINT)}}
$('#commentInput').removeClass('invalid').addClass('valid')}else{$('#commentInput').addClass('invalid').removeClass('valid')}});commentsCollapsible=$('#commentsCollapsible');commentsCollapsible.collapsible({accordion:!1});if(isOnline){run('commentsManager','getComments',{message:message,private:private||(typeof(RECIPIENT)!='undefined'&&RECIPIENT!=null)}).done((response)=>{console.info(response);commentsCollapsible=$('#commentsCollapsible');switch(response.status){case OK:let renderedHTML='';response.result.forEach((comment)=>{renderedHTML+=getRenderedComment(comment.id,comment.declaredName,comment.content,!1,comment.likes)});commentsCollapsible.append(renderedHTML);commentsCollapsible.slideDown();commentsCollapsible.collapsible({accordion:!1});if(commentsCollapsible.find('li').length>0){M.Collapsible.getInstance($('#commentsCollapsible')).open(0)}
...
function tryToPushRandomAd(then=()=>{}){run('adsManager','getRandomAd',{},()=>{},!0).done((response)=>{console.info(response);messages=$('.message');if(messages.length>0){if(response.result){messages.eq(Math.round(Math.random()*(messages.length-1))).after(getRenderedAd(response.result.id,response.result.content,response.result.companyName));reloadLayout()}else{console.warn('tryToPushRandomAd: unable to push, unexpected response.')}}else{console.info('tryToPushRandomAd: there are no messages in this view, this is a sad moment for advertisers. :(')}}).fail((error)=>{console.error(error)}).always(then)}
...
run('messagesManager','getRecent',{after:typeof(lastMessageData)!='undefined'?lastMessageData.message:undefined,startAt:typeof(lastMessageData)!='undefined'?lastMessageData.position:undefined,recipient:RECIPIENT,private:PRIVATE,sortBy:sortBy,nsfw:nsfwMode},()=>{isPullingChunks=!0}).done((response)=>{console.log(response);if(response.result.length>0){let renderedHTML='';response.result.forEach((message)=>{renderedHTML+=getRenderedMessage(message.id,message.content,message.declaredName,message.created,!0,parseInt(message.reported)==1,message.image,parseInt(message.verified)==1,message.comments,!0,message.likes,message.position)});row=gridContainer.find('.row');deferredActions=()=>{row.append(renderedHTML).fadeIn();reloadLayout(renderedHTML);if($('.message').last().position()['top']>document.documentElement.clientHeight&&deferredFetcher!=null){clearInterval(deferredFetcher);deferredFetcher=null}
console.info('tryToPullChunks: successfully pulled '+response.result.length+' chunks.');then();tryToPushRandomAd()};if(preloader.is(':visible')){preloader.fadeOut(()=>{preloader.hide();deferredActions()})}else{deferredActions()}}else{isPullingChunks=!0;canPullChunks=!1;console.info('tryToPullChunks: nothing to pull, shutting down...');deferredActions=()=>{displayRemovableWarning(PRIVATE?NO_MESSAGES_HINT:'¡Nada por acá, publicá primero!')}
if(firstCall){if(preloader.is(':visible')){preloader.fadeOut(()=>{preloader.hide();deferredActions()})}else{deferredActions()}}}
isPullingChunks=!1}).fail((error)=>{isPullingChunks=!1;console.error(error)})}}else{console.info('tryToPullChunks: you\'re offline, so you can\'t try to pull chunks now.')}}}
function getLastMessageData(){return $('*[data-message]').last().data()}
function cum(){$('*').animate({color:'white',backgroundColor:'white'})}
function tryToDeferAutoTooltip(){createMessageBtn=$('#createMessageBtn');createMessageBtn.tooltip();$(window).on('load',()=>{if(createMessageBtn.length>0){createMessageBtn.tooltip('open')}})}
let idleRunner=null;function initializeIdleRunner(){if(idleRunner!=null){clearInterval(idleRunner);idleRunner=null}
idleRunner=setTimeout(()=>{$('.tap-target').tapTarget('open');localStorage.setItem('hasSeenFeatures',!0);if($(window).scrollTop()>0){$('html, body').animate({'scrollTop':0},SCROLLTOP_DURATION)}},IDLE_TIMEOUT)}
function hasSeenFeatures(){return localStorage.getItem('hasSeenFeatures')!=null}
function goBackToTop(){$('html, body').scrollTop(0)}
...
let isReportingImpressions=!1;let reportedImpressions=[];document.addEventListener('scroll',()=>{try{if($('.tooltipped').length>0){$('.tooltipped').tooltip('close')}}catch(exception){}
if(reportAdImpressions){$('[data-ad]').each(function(){id=$(this).data('ad');if(isElementInViewport($(this)[0])&&!isReportingImpressions&&!reportedImpressions.includes(id)){run('adsManager','reportImpression',{id:id},()=>{isReportingImpressions=!0},!0).done((response)=>{console.info(response);if(response.status==OK){reportedImpressions.push(id)}}).fail((error)=>{console.error(error)}).always(()=>{isReportingImpressions=!1})}})}},{passive:!0});$('.sidenav').sidenav({onOpenStart:()=>{tooltips=$('.tooltipped');tooltips.tooltip();tooltips.tooltip('close')},onCloseEnd:()=>{createMessageBtn=$('#createMessageBtn');if(createMessageBtn.length>0&&createMessageBtn.hasClass('pulse')){createMessageBtn.tooltip('open')}}});$('#continueLoginBtn, #tryAccountRecoveryBtn').on('click',(event)=>{loginPassword=$('#loginPassword');loadRecaptcha();if(event.target.id=='continueLoginBtn'&&loginPassword.parent().is(':visible')){if(loginPassword.val().length>0){deferLoginPreloader();disable($('#tryAccountRecoveryBtn, #continueLoginBtn, #loginMailAddress, #loginPassword'));if(typeof(grecaptcha)=='undefined'){toast('No podemos validar tu sesión, parece que hay problemas con tu conexión.');enable($('#tryAccountRecoveryBtn, #continueLoginBtn, #loginMailAddress, #loginPassword'));return}
grecaptcha.ready(()=>{grecaptcha.execute(RECAPTCHA_PUBLIC_KEY,{action:'submit'}).then((token)=>{run('accountManager','tryLogin',{'mailAddress':$('#loginMailAddress').val(),'password':$('#loginPassword').val(),'token':token}).done((response)=>{console.log(response);switch(response.status){case SUSPICIOUS_OPERATION:toast('Necesitamos que resuelvas un desafío.');break;case NO_SUCH_ELEMENT:toast('El correo electrónico especificado no existe.');break;case NOT_ALLOWED:toast('La contraseña no es correcta.');break;case OK:$('#loginMailAddress, #loginPassword').removeClass('valid invalid').val('');animateRedirect(loginTarget==null?SYSTEM_HOSTNAME+'private/?fromLogin':loginTarget);break}}).always(()=>{enable($('#tryAccountRecoveryBtn, #continueLoginBtn, #loginMailAddress, #loginPassword'));stopPreloader()})}).catch((error)=>{console.warn('Failed to complete login due to a reCaptcha v3 server error.\n\nThe following information may help investigate the issue: \n\n'+error);enable($('#tryAccountRecoveryBtn, #continueLoginBtn, #loginMailAddress, #loginPassword'));stopPreloader()})})}else{markInvalid(loginPassword);stopPreloader()}}else{if($('#loginMailAddress').hasClass('valid')){deferLoginPreloader();run('accountManager','trySendLoginMail',{'mailAddress':$('#loginMailAddress').val(),'force':event.target.id=='tryAccountRecoveryBtn'},()=>{console.info('accountManager/trySendLoginMail: account creation/recovery started.');$('.tooltipped').tooltip('close');disable($('#tryAccountRecoveryBtn, #continueLoginBtn, #loginMailAddress, #loginPassword'))}).done((response)=>{console.log(response);switch(response.status){case BAD_REQUEST:toast('Algo anda mal, probá otra vez.');break;case ALREADY_EXISTS:loginPassword.parent().slideDown(()=>{loginPassword.focus().click()});$('#continueLoginBtn').html('Iniciar sesión');$('#tryAccountRecoveryBtn').fadeIn();break;case OK:$('#loginModal').modal('close');loginMailAddress=$('#loginMailAddress').val();noPasswordLoginModal=$('#noPasswordLoginModal');noPasswordLoginModal.find('.mailAddress').html(loginMailAddress);noPasswordLoginModal.find('.modal-footer a').attr('href','https://'+loginMailAddress.split('@')[1]);noPasswordLoginModal.modal('open');break;case ERROR:toast('Hubo un problema, probá otra vez.');break}}).always(()=>{enable($('#tryAccountRecoveryBtn, #continueLoginBtn, #loginMailAddress, #loginPassword'));$('#signupMailAddress').val('');$('.tooltipped').tooltip('close');stopPreloader()})}}});$('#loginBtn, #loginBtnMobile').on('click',()=>{$('.sidenav').sidenav('close');$('#loginModal').modal('open')});$('.modal').modal({onOpenStart:()=>{try{$('.tooltipped').tooltip('close')}catch(exception){console.warn('modal/onOpenStart: unable to close tooltips due to the following reason: \n\n',exception)}},onOpenEnd:pushFakeHistory});$('#commentsModal').modal({onOpenEnd:()=>{$('#commentsModal').find('.modal-content').scrollTop(0);pushFakeHistory()}});$('#loginModal').modal({onOpenStart:()=>{$('.tooltipped').tooltip('close')},onOpenEnd:()=>{$('#loginMailAddress').focus().click();pushFakeHistory()},onCloseEnd:()=>{$('#loginModal').css({bottom:''}).find('#loginForm').css({opacity:1}).find('.input-field').first().css({height:''});$('#loginForm').show();$('#loginStatus').html('');loginMailAddress=$('#loginMailAddress');loginMailAddress.val('');loginMailAddress.removeClass('valid invalid');loginPassword=$('#loginPassword');loginPassword.val('');loginPassword.removeClass('valid invalid');loginPassword.parent().hide();$('#tryAccountRecoveryBtn').hide();$('#continueLoginBtn').html('Continuar');createMessageBtn=$('#createMessageBtn');if(createMessageBtn.length>0&&createMessageBtn.hasClass('pulse')){createMessageBtn.tooltip('open')}
allowDeferredPreloader=!0}});$('#loginMailAddress, #loginPassword').on('keyup',(event)=>{if(event.key=='Enter'){$('#continueLoginBtn').click()}});$('#logoutBtn, #logoutBtnMobile').on('click',()=>{$('.sidenav').sidenav('close');if(isOnline){run('accountManager','tryLogout',undefined).done((response)=>{if(response.status==OK){animateRedirect(SYSTEM_HOSTNAME)}})}else{toast(NO_INTERNET_HINT)}});$('#loginMailAddress').on('keyup change',function(){value=$(this).val().trim();if(isMailAddressValid(value)){markValid($(this))}else{markInvalid($(this))}});$('#loginPassword').on('keyup change',function(){if($(this).val().length>0){markValid($(this))}else{markInvalid($(this))}});function registerServiceWorker(){if('serviceWorker' in navigator){navigator.serviceWorker.getRegistrations().then((registrations)=>{navigator.serviceWorker.register(SYSTEM_HOSTNAME+'serviceWorker.js').then(()=>{if(registrations.length>0){console.info('serviceWorker: the registered worker is up to date.')}else{console.info('serviceWorker: registration succeded.')}
checkIfOnline()}).catch((exception)=>{console.warn('We were unable to set up a service worker due to the following reason: \n\n',exception)})})}}
async function pushLoader(){console.log('common/window: success loading assets.');$('.custom-link').on('click',function(event){event.preventDefault();animateRedirect($(this).attr('href'))});$('input[type="text"], textarea').each(function(){if(typeof($(this).attr('data-length'))!='undefined'){$(this).characterCounter()}});if($('.tooltipped').length>0){$('.tooltipped').tooltip();$('.tooltip-content').addClass('thin')}
M.updateTextFields();if($('textarea').length>0){M.textareaAutoResize($('textarea'))}
wallpaper=$('.wallpaper');wallpaper.attr('src',wallpaper.attr('data-src'));$('.material-icons').removeClass('deferred-icon');loader();if(DO_NOT_TRACK){console.info('common/pushLoader: DNT header found, analytics tracking have been disabled for this session.')}else{console.info('common/pushLoader: Google Tag Manager will be injected in '+(IDLE_TIMEOUT/1000)+' seconds.');setTimeout(()=>{console.info('common/pushLoader: trying to load Google Tag Manager.');var script=null;script=document.createElement('script');script.src='https://www.googletagmanager.com/gtag/js?id='+GOOGLE_ANALYTICS_KEY;script.onload=setupGoogleAnalytics;script.onerror=()=>{console.warn('common/pushLoader: cannot track this session, we couldn\'t download GTM. Is the user having strict privacy settings without exposing a DNT header?\n\nFor more information, please refer to the console error above.')}
...
$('.brand-logo').on('click',()=>{if($(window).scrollTop()>=document.documentElement.clientHeight){goBackToTop()}else{animateRedirect(SYSTEM_HOSTNAME+(getStatusForNSFW()?'nsfw':''))}});$(window).on('resize',()=>{viewportThreshold=(VIEWPORT_VISIBLE_THRESHOLD*$(window).height())/100;$('.tooltipped').each(function(){tooltip=M.Tooltip.getInstance($(this));if(typeof(tooltip)!='undefined'&&tooltip.isOpen){tooltip.close()}})});window.addEventListener('popstate',function(event){let areModalsOpen=!1;$('.modal').each(function(){if($(this).is(':visible')){areModalsOpen=!0}});if(areModalsOpen){$('.modal').modal('close');gotHistoryPushState=!1}else{history.back()}});window.addEventListener('online',()=>{checkIfOnline(()=>{isOnline=!0;if($('.toast').length<1){toast(BACK_ONLINE_HINT)}
...
break}}
$('.sidenav-trigger, #nav-mobile, main').fadeIn();$('.tap-target').tapTarget({onOpen:()=>{$('.tap-target-origin').addClass('black-text')}})}
if(DISPLAY_GDPR_MODAL&&localStorage.getItem('acceptedGDPR')==null){main=$('main');main.css({filter:'blur(0.5rem)'});gdprModal=document.getElementById('gdprModal');M.Modal.init(gdprModal,{dismissible:!1});gdprModal=M.Modal.getInstance(gdprModal).open();$('#acceptCollectionBtn').on('click',()=>{localStorage.setItem('acceptedGDPR',!0);main.css({transition:MATERIALIZE_TRANSITION_TIME+'ms filter linear',filter:''});gdprModal.close();tryToDeferAutoTooltip();toast('¡Listo! Ya podés seguir navegando.');loadModulesAfterGDPR()})}else{tryToDeferAutoTooltip();loadModulesAfterGDPR()}
function setupNav(){let nav=$('nav');nav.pushpin();$('main').css({'padding-top':nav.height()});$('.dropdown-button').each(function(){M.Dropdown.init($(this)[0],{constrainWidth:!1,hover:!1})});userNavBackground=$('.sidenav .user-view .background');$(window).on('resize',()=>{if(userNavBackground!=null&&Object.keys(userNavBackground.data()).includes('backgroundUrl')&&window.innerWidth<993){userNavBackground.css({'background':'url('+userNavBackground.data('background-url')+')'});userNavBackground.removeData('backgroundUrl');console.log('setupNav: small screen detected, loading user-view background.')}});$(window).trigger('resize')}
