...
function getRenderedAd(id,content,companyName,created=null,image=null,forceClass=null,badgeData={bgClass:'bg-dark-10 bg-light-11',text:'Publicidad'},impressions=null,showRemoveButton=!1){return `<div class="col `+(forceClass==null?(isPrivate?'s12 m12 l6':'s12 m6 l3'):forceClass)+` message" data-ad="`+id+`">
...
function tryToPushRandomAd(then=()=>{}){run('adsManager','getRandomAd',{},()=>{},!0).then((response)=>{console.info(response);messages=$('.message');if(messages.length>0){if(response.data.result){messages.eq(Math.round(Math.random()*(messages.length-1))).after(getRenderedAd(response.data.result.id,response.data.result.content,response.data.result.companyName));reloadLayout()}else{console.warn('tryToPushRandomAd: unable to push, unexpected response.')}}else{console.info('tryToPushRandomAd: there are no messages in this view, this is a sad moment for advertisers. :(')}}).catch((error)=>{console.error(error)}).then(then)}
