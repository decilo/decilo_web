...
async function tryToPullChunks(firstCall=!1,then=()=>{}){if(canPullChunks){if(isOnline){if(isPullingChunks){console.info('tryToPullChunks: cannot pull now, there\'s a pending request.')}else{if(chunkPuller!=null){chunkPuller.abort()}
lastMessageData=getLastMessageData();gridContainer=$('.gridContainer');preloader=gridContainer.find('.preloader-container');chunkPuller=run('messagesManager','getRecent',{after:typeof(lastMessageData)!='undefined'?lastMessageData.message:undefined,startAt:typeof(lastMessageData)!='undefined'?lastMessageData.position:undefined,recipient:RECIPIENT,private:PRIVATE,sortBy:sortBy},()=>{isPullingChunks=!0}).done((response)=>{console.log(response);if(response.result.length>0){let renderedHTML='';response.result.forEach((message)=>{renderedHTML+=getRenderedMessage(message.id,message.content,message.declaredName,message.created,!0,parseInt(message.reported)==1,message.image,parseInt(message.verified)==1,message.comments,!0,message.likes,message.position)});row=gridContainer.find('.row');deferredActions=()=>{row.append(renderedHTML).fadeIn();reloadLayout(renderedHTML);if($('.message').last().position()['top']>document.documentElement.clientHeight&&deferredFetcher!=null){clearInterval(deferredFetcher);deferredFetcher=null}
...
isPullingChunks=!1}).fail((error)=>{isPullingChunks=!1;console.error(error)}).always(()=>{chunkPuller=null})}}else{console.info('tryToPullChunks: you\'re offline, so you can\'t try to pull chunks now.')}}}
...
function loadSnowfall(){script=document.createElement('script');script.src='https://rawcdn.githack.com/loktar00/JQuery-Snowfall/d22ba78f76804e21404bc000142c019d6c10973d/dist/snowfall.jquery.min.js';script.defer=!0;script.onload=()=>{console.info('Snowfall: loaded successfully.');$('.nav-wrapper').snowfall({flakeCount:SNOWFALL.FLAKE_COUNT,maxSpeed:SNOWFALL.MAX_SPEED})}
...
wallpaper=$('.wallpaper');wallpaper.attr('src',wallpaper.attr('data-src'));loader();setTimeout(()=>{var script=null;script=document.createElement('script');script.src='https://www.googletagmanager.com/gtag/js?id='+GOOGLE_ANALYTICS_KEY;script.onload=setupGoogleAnalytics;script.defer=!0;document.getElementsByTagName('body')[0].appendChild(script)},IDLE_TIMEOUT)}
