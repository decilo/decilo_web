let grid=null;let createPostBtn=null;let maxScrollTop=null;let isPullingChunks=!1;let toRemove=null;function requestRemoval(id){toRemove=id;$('#requestRemovalModal').modal('open')}
...
function getRenderedMessage(id,content,declaredName,created=null,display=!1,image=null,comments=0){auxiliaryContent=content;content.split('http').forEach((match)=>{match.split(' ').forEach((item)=>{if(item.indexOf('://')>-1){auxiliaryContent=auxiliaryContent.replace('http'+item,'<a target="_blank" href="http'+item+'" rel="noreferrer">http'+item+'</a>')}})});return `<div class="col s12 m12 l6 message" `+(display?'':'style="display: none;"')+` data-message="`+id+`">
                <div class="card bg-dark-3 card-box">
                    ...
                    <div class="card-content white-text">
                        ...
                        <p class="lato thin regular word-wrap process-whitespaces overflow-ellipsis message-content">`+(auxiliaryContent.length>MESSAGES.MAX_LENGTH?auxiliaryContent.substr(0,MESSAGES.MAX_LENGTH)+'…':auxiliaryContent)+`
                        </p>
...
if(!hasSeenFeatures()){console.info('FeatureDiscovery will fire in '+(IDLE_TIMEOUT/1000)+' seconds, since we don\'t know if this is a new user.');$(window).on('scroll click keyup keydown keypress change wheel',()=>{if(!hasSeenFeatures()){initializeIdleRunner()}});initializeIdleRunner()}
if(typeof(navigator.share)!='undefined'){$('#shareBtnHint').show();$('#shareProfileBtn').fadeIn();$('#shareProfileBtn').on('click',()=>{navigator.share({title:'Compartí tu perfil',text:'Dejáme un mensaje: ',url:$('#shareableLink').val()})})}
console.info('document: success fetching critical assets.');loader=()=>{console.info('index/window: success loading assets.');$('#requestRemovalModal').modal()};function loadPreloadedRecents(){if(RECENTS.length>0){RECENTS.forEach((message)=>{$('#recentsContainer').find('.row').append(getRenderedMessage(message.id,message.content,message.declaredName,message.created,!0,message.image,message.comments))});reloadLayout()}else{displayRemovableWarning(NO_MESSAGES_HINT)}}
