<?php
 function overflow32($v)
    ...
    function getRandomColor($img, $availableColors, $limit)
    {
        $colorBlocks = 
            str_split(
                $availableColors[mt_rand(0, $limit)], 2
            );

        foreach ($colorBlocks as &$colorBlock) {
            $colorBlock = hexdec($colorBlock);
        }
    ...
    $bgColor = getRandomColor($handle, $availableColors, $limit);

    for ($i = 0; $i < getShapeCount(); $i++) {
        $color = getRandomColor($handle, $availableColors, $limit);
